...
class Schedule extends Command
{
    ...
    {
        if($this->daemon){
            $this->output->writeln("<info>[daemon]Crontab is started successfully</info>");
            ...
            while (true) {
                ...
                if($process->isRunning()){
                    cache($this->getName(), $this->checkProcess(),60);
                }
                $process->wait();
    ...
    protected function makeProcess($task='cron:run',$connector=null, $memory = 128,$daemon=null)
    {
        ...
        return new Process(implode(" ",$command),ROOT_PATH?:env('ROOT_PATH'), null, null,null);
    }
